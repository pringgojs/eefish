...
function load_Modal(t){
	...
	ajax_Transfer(ajaxUrl, ajaxData, '#modal-output');
}
...
function ajax_Transfer(url, data, callback, asJson, hide = true){
	asJson = typeof asJson == 'undefined' ? false : true;
	

	if(asJson){
	    ajax_AsJson(url, data, callback,hide);
	} else{
	    ajax_AsXhr(url, data, callback,hide);
	}
}

function ajax_AsJson(url, data, callback,hide = true){
    var response, csrfToken;
	url = baseURL + url;
	if(url.substring(0,2) == '//'){
	    url = url.substring(1, url.length);
...
function ajax_AsXhr(url, data, callback,hide = true){
    var response, csrfToken;
	url = baseURL + url;
	if(url.substring(0,2) == '//'){
	    url = url.substring(1, url.length);
